...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [helloUserPlugin],
    });

    await request(server).get('/api/hello-user/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/hello-user/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/hello-user/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/hello-user/todos/${createdTodoItem.id}`)
